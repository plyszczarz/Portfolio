...
void ridge::Transform_a_pointRRR(vector3d &location, vector3d scale, vector3d trans)
{
...
/**
 * @brief Function that creates a specific obsticle called ridge hill, 
 * 
...
bool ridge::CreateHillRRR(istream &file_Entry_name, vector3d scale, vector3d pos, ostream &file_Exit_name)
{
  ...
  for (unsigned int Ind_krawedz = 0; Ind_krawedz < 5; ++Ind_krawedz) {
    for (unsigned int Ind_wierz = 0; Ind_wierz < 4; ++Ind_wierz) {
      ...
      if (coords[2] > 0.5) { // Tutaj realizowana jest deformacja
	if (coords[0] < 0) coords[0] = fabs(coords[0]);
      }
      
      Transform_a_pointRRR(coords, scale, pos);
...
bool ridge::CreateHillRRR(const char *file_Entry_name, vector3d scale, vector3d pos, const char *file_Exit_name)
{
  ...
  return CreateHillRRR(sfile_Entry_name, scale, pos, sfile_Exit_name);
}
...
double ridge::Checking(vector3d Olocation, vector3d Dlocation)
{
