...
void tableLand::Transform_a_pointTTT(vector3d &location, vector3d scale, vector3d trans)
{
...
/**
 * @brief Function that creates a specific obsticle called table land, 
 * 
...
bool tableLand::CreateHillTTT(istream &file_Entry_name, vector3d scale, vector3d pos, ostream &file_Exit_name)
{
  ...
  for (unsigned int Ind_krawedz = 0; Ind_krawedz < 5; ++Ind_krawedz) {
    for (unsigned int Ind_wierz = 0; Ind_wierz < 4; ++Ind_wierz) {
      ...
      Transform_a_pointTTT(coords, scale, pos);
...
bool tableLand::CreateHillTTT(const char *file_Entry_name, vector3d scale, vector3d pos, const char *file_Exit_name)
{
  ...
  return CreateHillTTT(sfile_Entry_name, scale, pos, sfile_Exit_name);
}
...
double tableLand::Checking(vector3d Olocation, vector3d Dlocation)
{
